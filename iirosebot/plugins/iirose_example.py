...
async def test_0(Message):  # 请保证同一个插件内不要有两个相同的指令函数名进行注册，否则只会保留最后一个注册的
    ...
    await API.send_msg_to_room('Hello World!')


@on_command('TEST-', True, command_type=[MessageType.room_chat, MessageType.private_chat])  # substring可输入布尔类型也可以是列表，布尔的情况下框架自动获取长度判断，用于取左侧的消息，第二个参数为数字类，框架会取这个数字的左侧，如果发送的消息=左侧这几个数字的消息就会执行此函数，函数需要有两个参数，第二个参数会返回去除指令的消息
async def test_1(Message, text):
    await API.send_msg(Message, text)

@on_command('echo ', True, command_type=[MessageType.room_chat, MessageType.private_chat])  # substring可输入布尔类型也可以是列表，布尔的情况下框架自动获取长度判断，用于取左侧的消息，第二个参数为数字类，框架会取这个数字的左侧，如果发送的消息=左侧这几个数字的消息就会执行此函数，函数需要有两个参数，第二个参数会返回去除指令的消息
async def echo(Message, text):
    if Message.user_id != "6533df3d933bf":
        return
    await API.send_msg(Message, text)
...
async def user_join_room(Message):
    ...
    # 框架默认消息均不排除自身，请通过Message中的is_bot进行判断，该参数为布尔类，为True时说明该消息为自身
